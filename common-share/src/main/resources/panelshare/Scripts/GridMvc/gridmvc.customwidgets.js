...
function CustomersFilterWidget() {
    ...
    */
    this.loadCustomers = function () {
        var $this = this;
        $.post("/Users/GetCustomersNames", function (data) {
    ...
    */
    this.fillCustomers = function (items) {
        var customerslist = this.container.find(".customerslist");
        for (var i = 0; i < items.length; i++) {
            customerslist.append('<option ' + (items[i] == this.value.filterValue ? 'selected="selected"' : '') + ' value="' + items[i] + '">' + items[i] + '</option>');
        }
    };
    /***
