(function (angular) {
    ...
    angular.module('FileManagerApp').service('apiHandler', ['$http', '$q', '$window', '$translate', '$httpParamSerializer', 'Upload',
        function ($http, $q, $window, $translate, $httpParamSerializer, Upload) {
            ...
            //MehdiB
            ApiHandler.prototype.setAuthHeader = function(){
            ...
            //MehdiB
            ApiHandler.prototype.getImageType = function(){
                return sessionStorage.getItem('fmanager-img-type') || 2;
            }

            ApiHandler.prototype.getTjoorPath = function (path) {
                return (sessionStorage.getItem('fmanager-burl') || '') + path;
            }
            ...
            ApiHandler.prototype.deferredHandler = function (data, deferred, code, defaultMsg) {
                ...
                if (data.result && data.result.error) {
                    data.result.error = $translate.instant(data.result.error)
                    this.error = data.result.error
                }
                if (!this.error && data.error) {
                    data.error.message = $translate.instant(data.result.error)
            ...
            ApiHandler.prototype.editImage = function (apiUrl, destination, files) {
                var self = this;
                //mehdib
                self.setAuthHeader();
                var deferred = $q.defer();
            ...
            //MehdiB            
            ApiHandler.prototype.downloadImage = function (imageUrl) {
                var deferred = $q.defer();
                self.inprocess = true;
                ...
                $http.get(imageUrl, { contentType: "image/png", dataType: "text", responseType : 'blob'}).then(function (response) {
